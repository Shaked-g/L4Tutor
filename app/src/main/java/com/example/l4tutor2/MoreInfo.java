...
public class MoreInfo extends AppCompatActivity {
    ...
    public void RegisterMoreInfo(View view) {
        ...
        if (cb_algebra1.isChecked())
            courses.add(String.valueOf(User.Courses.Linear_Algebra1));
        if (cb_algebra2.isChecked())
            courses.add(String.valueOf(User.Courses.Linear_Algebra2));
        if (cb_numbers.isChecked())
            courses.add(String.valueOf(User.Courses.Number_Theory));
        if (cb_infi1.isChecked())
            courses.add(String.valueOf(User.Courses.Infinitesimal_Calculus1));
        if (cb_infi2.isChecked())
            courses.add(String.valueOf(User.Courses.Infinitesimal_Calculus2));
        if (cb_discrete.isChecked())
            courses.add(String.valueOf(User.Courses.Discrete_Mathematics));
        if (cb_logic.isChecked())
            courses.add(String.valueOf(User.Courses.Logic_and_set_theory));
        if (cb_java.isChecked())
            courses.add(String.valueOf(User.Courses.Java));
        if (cb_data.isChecked())
            courses.add(String.valueOf(User.Courses.Data_Structures));
        if (cb_oop.isChecked())
            courses.add(String.valueOf(User.Courses.Object_Oriented));

        user.setDesiredCourses(courses);

        mDatabase.child(keyid).child("desiredCourses").setValue(user.getDesiredCourses());

        //moves to feed
        Toast.makeText(getApplicationContext(), "User added please login", Toast.LENGTH_SHORT).show();
        Intent intentFeed = new Intent(MoreInfo.this, LoginPage.class);
